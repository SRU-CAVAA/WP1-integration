...
class ContextualLayer(object):
    ...
    def __init__(self, stm=50, ltm=500, pl=20, load_ltm=False, sequential_bias=True, forget="NONE", value_function='default',
        ...
        self.sequential_bias = sequential_bias   # goal fidelity
        self.forget = forget
        ...
        self.enabled = False
        self.STM = [[np.zeros(pl), np.zeros(2)] for _ in range(self.ns)] # pl = prototype length. state, action
    ...
    def advance(self, e):
        ...
        if len(self.LTM[0]) > 0:
            ...
            if np.any(self.selected_actions_indx):
                ...
                self.action = [-1, -1] # Null action (indicates the agent that contextual do not generate a valid action)
    ...
    def update(self, couplet=[], reward=0):
        ...
        # Update trigger values. (Sequential bias update)
